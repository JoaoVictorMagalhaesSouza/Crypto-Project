...
class SplitData():
    ...
    def split_train_test(self, train_size=0.8):
        ...
        df_train = self.data.loc[:int(len_dataset*train_size)]
        df_test = self.data.loc[int(len_dataset*train_size):]
    ...
    def split_real_time(self):
        y_train = self.data.pop('future_price')
        train_dates = self.data.pop('date')
        X_train = deepcopy(self.data)
